...
dfComps <- data.frame(read_csv(url))
```
...
```{r}
# is.na is an operator which gives a true false indicator when a value in a specified vector is present and not present respectively. Found from stack overflow
...
```{r}
#which(dfComps$noURL == TRUE )
urlComps <- dfComps[which(dfComps$noURL == FALSE ),]
#urlComps <- dfComps[which(dfComps$noURL == TRUE ))
View(urlComps)
...
```{r}
str(urlComps)
# they have 2 variable  - funding rounds, funding year
```
...
```{r}
#na.rm inputs a logical value indicating whether value should be removed or not
mean(urlComps$funding_rounds, na.rm=TRUE)
```
...
# oldest company founded in 1900
```
...
```{r}
noURlComps <- dfComps[which(dfComps$noURL == TRUE ),]
mean(noURlComps$funding_rounds, na.rm =TRUE)
```
...
?str_replace
newVector<- str_replace(urlComps$permalink,'/organization/','')
```
...
```{r}
# funding total USD is the variable which is in Char but can be used as num
 vars = is.na(urlComps$funding_total_usd)
# we can check in vars, there is no true value thus there is no NA value
urlComps$newFundingTotalUSD <- as.numeric(urlComps$funding_total_usd)
## output:  NAs intrduced by force when we using as.numeric. Earlier we had no NA value.
```
...
```{r}
urlComps$newFundingTotalUSD <- as.numeric(urlComps$funding_new, na.rm=TRUE)
mean(urlComps$newFundingTotalUSD )
```
...
urlComps$funding_total_usd_new = str_replace_all(urlComps$funding_total_usd,' ','')
# commenting the code due to huge output
...
convertCharToNumNew <- function(char_string) {
  #put your code here
  step1 <- str_replace_all(char_string,' ','')
  step2 <- as.numeric(step1)
  return(step2)    
  }
...
```{r}
#THis is the variable name who's output was saved in P
#urlComps$funding_total_usd_new
```
...
```{r}
mean(convertCharToNumNew(urlComps$funding_total_usd_new), na.rm = TRUE)
mode("5")
"5"
