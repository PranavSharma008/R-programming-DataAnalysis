...
summary(cred)
str(cred)
# taking credit_risk as the factor variable 
# duration -- the duration of the credit taken
# amount -- amount of credit taken
# installment_rat -- installment rate which is applied on returning the credit initially taken
# present_residence -- number of people residing in the residence
# age -- age of the person taking the credit
# credit_history -- credit history of the person taking the rent
# people_liable -- number of dependent people
# credit_risk -- Credit risk factor associated displayed as either 1 or 2
```
...
```{r}
set.seed(109)
# taking p=0.66 as we are taking 2/3rd of the data for training and the rest for testing
trainList <- createDataPartition(cred$credit_risk,p=0.66, list= FALSE)
...
```{r}
#ksvm is the support vector machine model
credit_risky <- ksvm(credit_risk~.,data=training,C=5,cross=3,prob.model=TRUE)
```
...
```{r}
# the cross validation error is 30 percent; training error is 22 percent. We have kept cost C as 5
credit_risky
...
```{r}
# predicting the testing data with credit_risky calculated above.
svmPred <- predict(credit_risky,newdata=testing,type="response")
```
...
# the p value is negative with an accuracy of 70.59 percent. Though the no information accuracy rate is also 70 percent
```
...
```{r}
# accuracy is 0.7059 based on the confusion matrix
```
...
# the below value calculates the accuracy
sum(diag(table(svmPred,testing$credit_risk)))/sum(table(svmPred,testing$credit_risk))
```
...
```{r}
# 1) test dataset should be separate from the training dataset because the accuracy the model has with the test dataset is used as a measure how successful it would be when the application is deployed on Production. It is because the data which we get when the application is deployed into production is not used to train our data.
# 2) Ethical challenges may occur like discrimination in term of giving credit to people. 
```
...
library(rpart.plot)
rpart_credit_risk <- rpart(credit_risk~.,data=cred)
```
...
```{r}
# now here predict cred data with rpart_credit_risk. 
predict_rpart<-predict(rpart_credit_risk,newdata=cred, type='class')
confusionMatrix(predict_rpart,cred$credit_risk)
# accuracy is higher than before ie 73 percent. no information rate is same ie 70 percent. p-value is negative
```
...
```{r}
# using amount might now be appropriate. Also using age and duration would be inappropriate. The reason is because those data is an type of attribute which describes a person. There is no way to correlate the age to the credit_risk
```
